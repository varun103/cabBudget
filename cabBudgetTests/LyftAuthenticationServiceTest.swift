...
class LyftAuthenticationServiceTest: XCTestCase {
    ...
    func testAccessTokenExpirationGreater() throws{
        ...
        var lyftKeychainKey = LyftKeychainKey(name: "lyft", oauthToken: LyftOAuthToken(accessToken: "accessToken", refreshToken: "refreshToken", tokenType: "abc", expiration: 20, scope: "xyz", expirationTime: timePlusthirtySeconds))
        try lyftKeychainKey.saveInKeychain()
        try lyftKeychainKey.fetchFromKeychain()
        XCTAssertTrue(MockTest.accessTokenValid(lyftKeychainKey: lyftKeychainKey))
    ...
    func testAccessTokenExpirationLess() throws{
        ...
        try lyftKeychainKey.saveInKeychain()
        try lyftKeychainKey.fetchFromKeychain()
        XCTAssertFalse(MockTest.accessTokenValid(lyftKeychainKey: lyftKeychainKey))
        try lyftKeychainKey.removeFromKeychain()
    }
